...
function Stat({ label, value, valueColor }: { label: string; value: string; valueColor?: string }) {
  return (
    <div>
      <div style={{ fontSize: fontSize[12], color: colors.text.muted, fontWeight: fontWeight.semibold }}>{label}</div>
    ...
    >
      ...
      >
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: fontSize[14], color: colors.text.muted, marginTop: spacing[2] }}>
            {produtoNome} · {dataFmt}
          </div>
        </div>
      ...
      >
        ...
        <Stat label="Comissão" value={comissaoFmt} valueColor={colors.success.trend} />
      </div>
    </div>
  );
}
